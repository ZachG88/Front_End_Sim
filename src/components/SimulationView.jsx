import { LANE_CONFIGS } from '../hooks/useSimulation.js';
import LaneRow from './LaneRow.jsx';

function LaneGroup({ type, lanes }) {
  const cfg = LANE_CONFIGS[type];
  if (!lanes.length) return null;
  const totalServed = lanes.reduce((s, l) => s + l.totalServed, 0);
  const totalQueued = lanes.reduce((s, l) => s + l.queueLength, 0);
  const busyCount   = lanes.filter(l => l.isBusy).length;

  return (
    <div className="lane-group">
      <div className="lane-group-header">
        <div className="lane-group-bar" style={{ background: cfg.color }} />
        <div>
          <div className="lane-group-name">{cfg.name}</div>
          <div className="lane-group-desc">{cfg.description}</div>
        </div>
        <div className="lane-group-meta">
          <span className="lg-active">{busyCount}/{lanes.length} active</span>
          <span className="lg-badge" style={{ background: cfg.dimColor, color: cfg.color }}>
            {totalServed} served · {totalQueued} queued
          </span>
        </div>
      </div>
      {lanes.map((lane, i) => <LaneRow key={lane.id} lane={lane} laneIndex={i} />)}
    </div>
  );
}

export default function SimulationView({ display }) {
  if (!display) return <div className="sim-view"><div className="empty-state">Press Start to begin the simulation</div></div>;

  const { lanes, finished, stats } = display;
  const byType = {
    traditional:     lanes.filter(l => l.type === 'traditional'),
    'self-checkout': lanes.filter(l => l.type === 'self-checkout'),
    prescan:         lanes.filter(l => l.type === 'prescan'),
  };

  if (!lanes.length) return <div className="sim-view"><div className="empty-state">Add lanes in the control panel to begin</div></div>;

  return (
    <div className="sim-view">
      {finished && (
        <div className="finished-banner">
          <div style={{ flex: 1 }}>
            <div className="finished-title">Simulation Complete</div>
            <div className="finished-sub">
              {stats.served} members served · avg wait {display.stats.avgWaitStr} · peak wait {display.stats.maxWaitStr}
            </div>
          </div>
        </div>
      )}
      <LaneGroup type="traditional"   lanes={byType['traditional']} />
      <LaneGroup type="self-checkout" lanes={byType['self-checkout']} />
      <LaneGroup type="prescan"       lanes={byType['prescan']} />
      <div className="sim-footer">
        <span>{lanes.length} lanes total · {lanes.length - stats.totalServing} idle</span>
        <span>
          {['traditional', 'self-checkout', 'prescan']
            .filter(t => byType[t].length > 0)
            .map(t => `${byType[t].length} ${LANE_CONFIGS[t].name}`)
            .join(' · ')}
        </span>
        <span>Policy: {stats ? 'Shortest Queue' : '—'}</span>
      </div>
    </div>
  );
}
